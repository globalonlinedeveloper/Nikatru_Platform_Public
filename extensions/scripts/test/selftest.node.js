...
const NO_CASE_RECORDED = [
  ...
  { gate: 'scripts/test/hang-guard.test.mjs',
    why: 'PERMANENT, recorded 2026-09-06. Like this file, it is a suite rather than a gate — node:test, run by the same `selftest` job. What grades it is that each of its cases is a proven pair, and one of them found a real defect in the guard on the day it was written: the SIGKILL the ceiling issues raises its own `exit` event, which beat the ceiling to the verdict and reported a wedged process as exit 1 instead of 124.' },
  { gate: 'scripts/lib/hang-guard.mjs',
    why: 'COVERED ELSEWHERE, recorded 2026-09-06 — this is not an open gap. Its red/green pairs are in scripts/test/hang-guard.test.mjs (a command exiting 0 passes through as 0; one that never exits is killed at the ceiling and reported 124, with Node\'s diagnostic report on disk to prove the signal landed; one exiting 3 comes back as 3, not 1 and not 124; a report still being written at the ceiling is waited for, and one the grace cannot wait out is named INCOMPLETE rather than collected, added 2026-09-26 after run 36204413291 read a report that did not parse, one the guard had listed as collected), and the `selftest` job runs them in this same job. They cannot live HERE: run() appends `--repo-root <root>`, the guard forwards everything after `--` to the child verbatim, and `node -e ... --repo-root x` answers "bad option: --repo-root" and exits 9 — measured 2026-09-06. So every case through this file\'s runner would grade node\'s argument parser, not the ceiling.' },
  ...
  { gate: 'scripts/changelog-section.mjs',
    why: 'OPEN GAP, recorded 2026-08-25. Called only from release.yml, to cut one version section out of a CHANGELOG for the release body. Uncovered means a release note that silently comes out empty is caught by nobody. COVERED ELSEWHERE since 2026-09-19 — no longer an open gap. tooling/ci/test/extensions-shared-constants.test.mjs RUNS this gate on a synthetic tool tree: every heading spelling changelogTop() recognises must yield the notes, an EMPTY section must exit 1 with no output, a missing version must exit 1, and a CHANGELOG with no recognisable heading must exit 2 (mutation-proven: disabling the empty-section refusal reddens it). Not here for the same reason as check-contracts-sync: that case needs its own tool tree, not the --repo-root tree every case in this file mutates.' },
