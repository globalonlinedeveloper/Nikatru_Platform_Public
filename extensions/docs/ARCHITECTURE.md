...
| The shared runtime | `core/` — and `core/core.json` is its own status file, module by module | **Partial.** Its version, each module's status and the counts are in `core/core.json`, not here. Every file in it was promoted from code that already ran. See [CORE-POLICY.md](CORE-POLICY.md#5-status-what-exists-today). |
