...
- **Zero network calls** — no `fetch`, `XMLHttpRequest`, `WebSocket`, `sendBeacon`, or `EventSource` anywhere. (The only "fetch" strings are code comments.) **Since 2026-08-26 the browser enforces this too on CHROMIUM, and not only this audit — on FIREFOX it does not, and the source scan is still the whole fence there (§B.1.4)** — `manifest.json` declares `content_security_policy.extension_pages` with `connect-src 'none'`. Read **§B.1** before leaning on that sentence: it states exactly what the directive does and does **not** reach, and content scripts are outside it. ⏱ **2026-09-24 (G8): the Firefox half of that sentence no longer holds.** `publish/manifest.firefox.json` no longer deletes `content_security_policy`, so the Firefox package carries the SAME policy — `scripts/policy-check.mjs` fails an overlay that names the key and `publish/verify-firefox-package.node.js` fails a merged or packed manifest whose policy differs. The policy also gained `default-src 'none'` and `style-src 'self'` (O-FULLSHOT-CSP-HAS-NO-DEFAULT-SRC); `test/e2e/network-audit.mjs` observes the Chromium pages at run time and `test/e2e/gecko-smoke.mjs` the Firefox ones.
...
| A4 | Default MV3 Content Security Policy (no `unsafe-eval`, no remote script) | ✅ | No `content_security_policy` override in `manifest.json` → strict MV3 default applies ⚠️ **SUPERSEDED 2026-08-26** — see **§B.1**: `manifest.json` now declares an explicit `content_security_policy.extension_pages`. It is **stricter than the MV3 default in six directives, identical in one (`script-src`) and looser in none**, so what A4 actually requires (no `unsafe-eval`, no remote script) still holds — `script-src 'self'` is now stated rather than inherited. ⚠️ **That sentence describes the CHROMIUM package only.** The Firefox package deliberately carries **no** `content_security_policy` at all, so A4 as originally written stays literally true of the Gecko build — see **§B.1.4**. ⏱ **2026-09-24 (G8):** the Gecko build now inherits the Chromium policy unchanged — `default-src 'none'; script-src 'self'; object-src 'none'; img-src 'self' data: blob:; style-src 'self'; connect-src 'none'; frame-src 'none'; base-uri 'none'; form-action 'none'` — so A4's "stricter than the MV3 default, looser in none" is now true of BOTH packages. `data:` stays in `img-src`: the result page stitches frames that `captureVisibleTab` returned as `data:image/png` URLs, and removing it stopped the result page rendering (measured 2026-09-24, Chromium build 1194). |
...
| A7 | `minimum_chrome_version` declared | ✅ | `"116"` (optional but good practice; MV3 + optional host perms supported). Chrome/Edge only — deliberately **absent** from `publish/manifest.firefox.json`, where Gecko's `strict_min_version: "140.0"` does the job (raised from 128.0 on 2026-09-24, EXT-3: 140 is the first desktop Firefox with built-in data-collection consent) |
...
⏱ **2026-09-25 (O-FULLSHOT-CSP-HAS-NO-DEFAULT-SRC): closed.** The seven `<style>` blocks, the inline
`style=""` attributes and the one style `pages/editor.js` wrote into `innerHTML` moved into
`pages/<page>.css`, and the policy now declares `default-src 'none'` and `style-src 'self'`, with every
other directive above still stated explicitly and `data:` kept in `img-src`. `scripts/policy-check.mjs`
fails a page that carries an inline style and a policy that drops `default-src`. The three paragraphs
above are the record of the day inline styles were unrestricted.
