...
/// exist.
///
/// 🔴 WHAT WAS BROKEN, MEASURED. `sendPasswordReset` was implemented and a real
/// recovery mail was delivered on 2026-08-03. There was no `updatePassword` on
/// the seam, no `/reset-password` in Subly's 19 routes or the brick's 9, and no
/// `redirectTo` on the request — so the mail arrived, the link resolved to the
/// project's Site URL, and the person who followed it was dropped on the home
/// screen with nothing to set. A user who forgot their password could ask for
/// help and then had no way to accept it, having spent their one link.
///
...
class _ResetPasswordScreenState extends ConsumerState<ResetPasswordScreen> {
  ...
  Future<void> _submit(core.AuthRepository auth, AppLocalizations l10n) async {
    if (_busy) return;
    // 🔴 CHECKED BEFORE THE NETWORK, and the mismatch arm is the reason it has
    // to be. A confirmation typo is the one error on this screen the SERVER
    // cannot catch: both fields hold well-formed passwords, so gotrue accepts
    // the wrong one and the user is locked out of the account they just
    // recovered — with their single-use link already spent.
  ...
  /// The single exit, from every state.
  ///
  ...
  List<Widget> _body(
    ...
    // landed on `/` and was bounced to `/sign-in` with no explanation, while
    // this paragraph called itself "the state the feature reaches most often in
    // the field". `passwordResetArrivalProvider` reads the launch URL and the
    // seam's `recoveryLinkFailed` event; the router sends both here.
    //
    ...
    return <Widget>[
      ...
      TextField(
        ...
        // to OFFER TO SAVE rather than autofill the old one — which on this
        // screen would refill the very password the user cannot remember.
