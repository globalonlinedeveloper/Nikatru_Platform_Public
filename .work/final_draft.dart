...
void main() {
  ...
  test(
    'the budget leaves room for the weekly digest',
    () async {
      // The digest holds one fixed id (`_digestId`) in the same per-app pool,
      // so the renewal budget must be strictly below the platform limit — a
      // renewal set filling all 64 would push the digest out.
      expect(
        NotificationService.renewalReminderBudget,
        lessThan(_iosPendingLimit),
  ...
  test(
    'the kept reminders are the SOONEST ones, whatever order they arrive in',
    () async {
      final NotificationService service = await readyService();

      // Shuffled deterministically: the repository gives no ordering guarantee,
      // so a cap that just took `.take(N)` of the incoming list would keep an
      // arbitrary set and drop renewals happening this week.
      final List<Subscription> subs = _subs(80).reversed.toList();
      await service.syncAll(subs, copy: _copy);
  ...
  test(
    'a set inside the budget is scheduled in full',
    () async {
      final NotificationService service = await readyService();
      ...
      expect(
        scheduled().length,
        10,
        reason: 'the cap must not touch the ordinary case',
      );
    },
  );

  test(
    'Android is not capped — it has no such limit',
    () async {
      // ⚠️ Capping a platform that does not need it is a regression, not a fix.
      // Android schedules through AlarmManager, which has no 64-request pool.
      ...
      expect(scheduled().length, 80);
    },
    // The plugin's platform implementation is chosen once per process from
    // `defaultTargetPlatform`; flipping the override afterwards does not
    // re-pick it. See the note in the test body.
    skip: 'see reminder_budget_android_note below',
  );
}
