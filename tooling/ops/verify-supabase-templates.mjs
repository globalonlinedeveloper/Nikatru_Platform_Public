...
//       against the record. With SELFHOSTED_SUPABASE_URL unset the URL is
//       BOXC_DEFAULT_TARGET (selfhosted-auth.mjs, the preflight's default too),
//       and the credentials line says which one was used;
...
async function verifySelfHosted() {
  ...
  // (b) the settings endpoint. Credentials from the ENVIRONMENT only. The URL
  // defaults to the Box C origin the preflight uses (BOXC_DEFAULT_TARGET): the
  // vault still holds a retired host under another name, and it does not resolve.
  const fromEnv = Boolean(process.env.SELFHOSTED_SUPABASE_URL);
  const url = fromEnv ? process.env.SELFHOSTED_SUPABASE_URL : BOXC_DEFAULT_TARGET;
  const anonKey = process.env.SELFHOSTED_SUPABASE_ANON_KEY;
  const cred = [
    fromEnv ? `auth URL ${fingerprint(url)} (from SELFHOSTED_SUPABASE_URL)` : `auth URL ${url} (default)`,
  ...
  if (!anonKey) {
    add('(b) settings', {
      ...
      verdict: 'LOST',
      detail: 'SELFHOSTED_SUPABASE_ANON_KEY is not in the environment — the live settings were not read',
    });
  } else {
