...
const UNCOVERED_TARGETS = {
  Chrome: 'The Chrome Web Store extension lives in the Project_Cross_browser_Extensions repository pair. CORRECTED 2026-08-19: this used to read "outside the store tree entirely" and that is no longer true — the pair now sits INSIDE the store tree at Projects/Chrome_Web_Store/Chrome/Extensions/Nikatru_Chrome_Extensions_{Public,Private}, moved and renamed by the Store x Platform x Type reorg (the GitHub repos kept their old names). It is still outside THIS repository, which is what makes the target uncovered here, and tooling/channel-register.json has never described a browser-extension channel. catalog/store-matrix.json backing.products[fullshot] records the slot as shell-claimed for exactly this reason: the product exists and is not filed here. Whether one repo becomes three is STORE-MATRIX-PLAN.md section 5.2 and it is OPEN.',
