...
//
//   9 · every provider the app's configuration triggers is declared — each row
//       of tooling/legal/provider-register.json carries `requiredWhen`, and
//       tooling/legal/required-providers.mjs evaluates it against the app's
//       app.yaml and the laned app channels of tooling/channel-register.json.
//       A required provider missing from privacy.yaml `processors` fails,
//       naming the trigger and the file:line of the fact that fired it; a
//       declared `never` provider fails; a row with no `requiredWhen`, or a
//       kind the resolver does not know, is COVERAGE LOST.
//       O-APP-PRIVACY-OMITS-STORE-BILLING.
//
...
// ── limb 9 · every provider the app's configuration triggers is declared ────
// O-APP-PRIVACY-OMITS-STORE-BILLING. app.yaml gained `billing.mobileIap` while
// privacy.yaml went on naming five processors. Limb 3 checks that each id a
// privacy.yaml names is a register row; nothing asked which rows the app's own
// facts make necessary. That rule lives on each register row (`requiredWhen`)
// and tooling/legal/required-providers.mjs evaluates it; this limb reads the
// three inputs, compares, and points at the fact that fired.
{
  const CHANNELS = 'tooling/channel-register.json';
  // One read, never `existsSync` then `readFileSync` (CodeQL js/file-system-race):
  // "is it there" and "what does it say" are the same observation. Anything other
  // than ENOENT or EISDIR is re-thrown, so an unreadable file is never "absent".
  const readText = (rel) => {
    try {
  ...
  const providerText = readText(PROVIDERS);
  const channelText = readText(CHANNELS);
  if (providerText === null || channelText === null) {
    coverageLost([
      `limb 9 cannot read ${providerText === null ? PROVIDERS : CHANNELS}.`,
      'Which providers an app must declare is a function of the provider register and the channel register;',
      'without either, every privacy.yaml would be graded against nothing.',
    ]);
  }
  let providerRegister;
  let channelRegister;
  try {
    providerRegister = JSON.parse(providerText);
    channelRegister = JSON.parse(channelText);
  } catch (e) {
    coverageLost([`limb 9 could not parse ${providerRegister === undefined ? PROVIDERS : CHANNELS} (${e.message}).`]);
  }
  const problemsBeforeRequired = problems.length;
  const neverIds = new Set(neverProviders(providerRegister));
  let appsGraded = 0;
  let requiredCount = 0;
  let declaredCount = 0;
  for (const { id, doc: appYaml } of declarations) {
    const privacyRel = `${APPS_DIR}/${id}/privacy.yaml`;
    const privacyText = readText(privacyRel);
    if (privacyText === null) continue;
    let privacy;
    try {
      privacy = parseYaml(privacyText);
    } catch {
      continue; // limb 3 reports the parse error; there is no declared set to compare
    }
    let required;
    try {
      required = resolveRequiredProviders(id, { appYaml, channelRegister, providerRegister });
    } catch (e) {
      if (!(e instanceof RequiredWhenError)) throw e;
      if (e.code === 'UNKNOWN_KIND') {
        coverageLost([
          `limb 9 cannot judge ${PROVIDERS}: a row's requiredWhen is missing or of a kind the resolver does not know.`,
          ...e.findings,
          `The kinds tooling/legal/required-providers.mjs judges are ${REQUIRED_WHEN_KINDS.join(', ')}.`,
        ]);
      }
      for (const f of e.findings) problems.push(`${PROVIDERS}: ${f}`);
      break;
    }
    appsGraded += 1;
    const declared = new Set(
      (Array.isArray(privacy?.processors) ? privacy.processors : []).map((p) => p?.id).filter((x) => typeof x === 'string'),
    );
    requiredCount += required.length;
    declaredCount += declared.size;
    const appYamlRel = `${APPS_DIR}/${id}/app.yaml`;
    const appYamlText = readText(appYamlRel) ?? '';
    for (const r of required) {
      if (declared.has(r.id)) continue;
      const facts = [];
      if (r.appYamlPath) facts.push(`${appYamlRel}:${yamlKeyLine(appYamlText, r.appYamlPath) ?? '?'} (${r.appYamlPath})`);
      for (const c of r.channels ?? []) facts.push(`${CHANNELS}:${jsonIdLine(channelText, c.id, '"channels": [') ?? '?'} (${c.id} ${c.via})`);
      if (facts.length === 0) facts.push(`${PROVIDERS}:${jsonIdLine(providerText, r.id, '"providers": [') ?? '?'} (requiredWhen ${r.kind})`);
      problems.push(
        `${privacyRel}: processor "${r.id}" is required and not declared — ${r.trigger}. Fact: ${facts.join('; ')}. ` +
          'Add it to `processors` with its purpose, source and asOf, then re-render the notice.',
      );
    }
    for (const d of declared) {
      if (!neverIds.has(d)) continue;
      const why = providerRegister.providers.find((p) => p.id === d).requiredWhen.why;
      problems.push(
        `${privacyRel}: processor "${d}" is declared, and its row at ${PROVIDERS}:${jsonIdLine(providerText, d, '"providers": [') ?? '?'} ` +
          `says requiredWhen never ("${why}"). Remove the processor, or change that row's requiredWhen and its reason.`,
      );
    }
  }
  if (appsGraded === 0 && problems.length === problemsBeforeRequired) {
    coverageLost([
      `limb 9 graded no app: none of the ${declarations.length} declaring app(s) has a readable privacy.yaml.`,
      'Every triggered provider would then pass as declared over an empty set.',
    ]);
  }
  if (problems.length === problemsBeforeRequired) {
    ok(
      `limb 9 — ${appsGraded} app(s) graded: ${requiredCount} required provider(s), each declared among ${declaredCount} ` +
        'processor(s), and no `never` provider declared',
    );
  }
}
