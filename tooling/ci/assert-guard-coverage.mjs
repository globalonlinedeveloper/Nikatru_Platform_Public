...
const NOT_A_SCANNER = new Map([
  ...
  [
    'run-page-anchor.mjs',
    'is not a guard: it is the ONE answer to "is this page of GitHub Actions run history CURRENT, or a stale replica\'s page?" — the self-run floor (GITHUB_RUN_ID of the running workflow on the running ref), the branch-head anchor (main HEAD from the commits endpoint, for a workflow `pushTriggersBranch` reads as run on every push), the cross-read by creation date, and `judgeRunPage`, which returns `stale page — …` when one is violated. Pure functions: pages, env and a commit body in, a verdict out; no network, no filesystem, no exit. Extracted 2026-09-18 (coverage unit stale-run-page) when GitHub served the SAME stale page to both widths of reconcileRunReads three times in one day — ops-watch run 35369631763, PR #806\'s CI and the platform Worker watchdog — so two node readers share it rather than each growing a copy: assert-ops-register.mjs (every shared branch page, through anchoredBranchPage) and assert-platform-proof-fresh.mjs (through anchoredRuns). Each turns the refusal into its OWN unreadable / COVERAGE LOST, so "did my read reach a current history" belongs to them. Its failing cases are in test/run-page-anchor.test.mjs: the three measured pages with their real run ids, each beside a GREEN CONTROL, reached THROUGH both callers, plus a wiring case per caller that reds when a reader stops calling it (mutation-proved 2026-09-18). It sits flat in tooling/ci because the stray-.mjs check above (correctly) treats a subdirectory as a guard escaping the scan.',
  ],
]);
