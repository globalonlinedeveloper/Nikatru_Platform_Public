...
const NOT_A_SCANNER = new Map([
  [
    'apple-provisioning.mjs',
    'is not a guard: it is the ONE reading of "what does a declared Apple capability require, and do the account and the tree agree with it" (O-STAMP-APPLE-PROVISIONING-MANUAL, [ADR 088]) — the tooling/apple-provisioning.json validation, the Runner.entitlements derivation, the flat-plist reader that refuses what it cannot read instead of skipping it, the three-way entitlement comparison, the App ID capability judgement (which never answers settled over a missing grouping or a missing DECLARED_AGE_RANGE), the profile plan and the profile-set zip writer. Pure functions: register, text, bytes and API read-backs in, verdicts out; no filesystem, no network, no exit. Two callers share it so they cannot disagree about what a capability requires — tooling/ci/assert-apple-entitlements.mjs (ci.yml, the tree) and tooling/ops/provision-apple.mjs (the vault key, the account) — and each carries its own COVERAGE LOST over what it read. Its failing cases are in test/apple-provisioning.test.mjs, which reds every refusal against the REAL register and a copy of the REAL tree, with a green control beside each. It sits flat in tooling/ci because the stray-.mjs check above (correctly) treats a subdirectory as a guard escaping the scan.',
  ],
  [
    'channel-surface.mjs',
    'is not a guard: it is the ONE answer to "does this channel\'s surface ship a Flutter application?", read off tooling/channel-register.json `surfaces.<name>.flutterApp` — `declaredSurfaces`, `flutterAppChannel` (true, false, or NULL for a surface nobody declared), `partitionByFlutterApp` and the shared refusal sentence. Pure functions over a parsed register: no filesystem, no tree, no exit. Extracted 2026-09-15 (O-EXT-SURFACE-AXIS) when six guards — assert-channel-claims, assert-channel-register, assert-platform-proof-fresh, assert-purchase-path, assert-seams-wired and release-manifest — each spelled that question as the literal `surface !== \'extension\'`, so a third surface was filed as a Flutter app by all six at once. The coverage question belongs to those callers, each of which turns NULL into its own COVERAGE LOST, and assert-channel-register refuses a surface that declares no boolean. Its failing cases are in test/channel-surface.test.mjs, and each caller\'s suite carries a third-surface case. It sits flat in tooling/ci because the stray-.mjs check above (correctly) treats a subdirectory as a guard escaping the scan.',
  ],
  [
    'price-figure.mjs',
    'is not a guard: it is the ONE answer to "does this text name a price, or the lifetime plan?" — `PRICE`, the union of the two matchers assert-no-price-literals.mjs (symbol-first and ISO-code prices in .dart) and assert-store-metadata.mjs (₹, Rs, INR, $ and USD in the apps-gov-in listing) each carried until 2026-09-24, and `LIFETIME` ([ADR 093] §2, §11.2). Two regexes: no filesystem, no tree, no exit. Extracted (O-PRICE-GUARD-IS-DART-ONLY) because neither old matcher was a superset of the other — `€4.99` passed the listing matcher and `Rs 2,499` passed the dart one — so both guards import it and cannot disagree about what a price is. The coverage question belongs to those two importers, each of which carries its own COVERAGE LOST over what it reads, and assert-no-price-literals.mjs proves the matcher against known-dirty input on every run before it trusts it. Its failing cases are in test/price-figure.test.mjs. It sits flat in tooling/ci because the stray-.mjs check above (correctly) treats a subdirectory as a guard escaping the scan.',
  ],
  [
  ...
  [
    'adapter-set.mjs',
    'is not a guard: it is the ONE answer to "which packages are adapters, and which vendors does each one wrap" - the pubspec dependency reader and the adapter derivation (a package under packages/ that is not core or design_system and declares a third-party dependency: not sdk:, not path:, not nikatru_*, not *lints). Pure functions: a repo root in, a derived set out; no verdict, no exit. It was extracted on 2026-09-19 because assert-adapter-capabilities.mjs claimed "the same derivation" as assert-package-boundaries.mjs while carrying its own copy with a different SDK rule. Both import it now, and each carries its own COVERAGE LOST floor over what it returns (MIN_WRAPPED, MIN_ADAPTERS). Its failing cases are in test/adapter-set.test.mjs, which also runs BOTH guards over copies of the real pubspecs and requires the same adapter set from each; restoring the old private reader in assert-package-boundaries.mjs turned that case red. It sits flat in tooling/ci because the stray-.mjs check above (correctly) treats a subdirectory as a guard escaping the scan.',
  ],
  [
    'sql-seed.mjs',
  ...
  [
    'signing-seam.mjs',
    'is not a guard: it is the ONE copy of the signing primitives the per-keyKind signing scripts share (O-SIGNING-PRIMITIVES-IN-FOUR-COPIES) — the all-or-none secret-set law, the release-lane derivation (the ref from GITHUB_REF and GITHUB_WORKFLOW_REF, and the arming verdict IMPORTED from channel-arming.mjs rather than restated), the base64 decode with its round-trip and format-marker check, key-material placement at an absolute path with a mode, and the $GITHUB_ENV export that refuses a value with a line break. It holds no channel id, spawns nothing and exits nowhere: it writes only to the paths its caller hands it, and a refusal goes back to the caller as a verdict or through the caller\'s own `fail`, so each adapter\'s COVERAGE LOST stop — and the exit 2 limb 2b reads in it — stays in the adapter\'s file. "Did my scan still reach the tree" belongs to its importers — apple-signing.mjs since 2026-09-25, android-signing.mjs and appimage-signing.mjs when they are moved onto it — each of which exits COVERAGE LOST when its own register row is absent. Its own failing cases are in test/signing-seam.test.mjs, a refused input beside a passing one for every primitive; test/signing-seam-census.test.mjs fails on an adapter that carries its own copy of a primitive and is not listed there as PENDING (android and appimage, until C5b), and on a channel id, an exit or a spawn in this file. It sits flat in tooling/ci because the stray-.mjs check above (correctly) treats a subdirectory as a guard escaping the scan.',
  ],
  [
  ...
  [
    'worker-routes.mjs',
    'is not a guard: it is the ONE reading of "what does this Hono Worker mount" — the comment stripper that keeps string literals, and the mount walk that follows `app.route()` into imported sub-routers and into in-file `new Hono` groups. Moved VERBATIM out of assert-platform-register.mjs on 2026-09-26 (O-SERVICE-KIT-UNBUILT, E-a1), because tooling/scripts/provision-backend.mjs step [6] writes a new app Worker\'s `appWorkers` row with its `routes` derived from the stamped entrypoint, and a second copy of the parser would give that row and the guard two answers to one question. Pure functions over the one root and file its caller hands it: it walks no directory, never exits, and reports what it could not follow on the `notes` array it is given. So "did my scan still reach the tree" belongs to its callers: assert-platform-register.mjs keeps its own COVERAGE LOST on zero mounts and on no sub-router followed, printing those notes, and step [6] refuses a Worker in which it found no mount. Its own failing cases are in test/worker-routes.test.mjs (an unresolvable mount, a missing route file, no `new Hono`, the `/*`-in-a-string and trailing-slash shapes, a root that is not the working directory), with a green control and the real brick template as the tree case. It sits flat in tooling/ci because the stray-.mjs check above (correctly) treats a subdirectory as a guard escaping the scan.',
  ],
  [
    'text-reductions.mjs',
  ...
  [
    'd1-stores.mjs',
    'is not a guard: it is the ONE D1 walk — which live wrangler configs exist (tooling/bricks excluded, and the exclusion reported) and which D1 databases each OWNS (the config that declares `migrations_dir`), with the tables their migrations create. Moved out of assert-retention-coverage.mjs on 2026-09-26 (O-SERVICE-KIT-UNBUILT, E-b2) so that assert-retention-coverage.mjs and tooling/scripts/provision-backend.mjs --check enumerate the same owned databases instead of two walks that could disagree. It never exits and prints nothing: every place its walk could come up short (a migrations_dir that does not exist, one with no .sql file, a config that does not parse) is handed to the caller\'s `lost` callback, and each caller turns that into its own COVERAGE LOST (exit 2); an empty result is the caller\'s COVERAGE LOST too. Its failing cases run through its callers: test/retention-coverage.test.mjs and test/provision-backend.test.mjs (K6-K9). It sits flat in tooling/ci because the stray-.mjs check above (correctly) treats a subdirectory as a guard escaping the scan.',
  ],
  [
  ...
  [
    'tool-output.mjs',
    'is not a guard: it is the ONE block a guard prints when an EXTERNAL tool\'s return is why it stopped — `whatToolReturned` gives the tool\'s path, its version, how it exited and the first 20 lines of stdout and of stderr, and `toolOutputLines` is the per-stream half. Pure functions: strings and a spawnSync-shaped result in, lines out; it runs nothing, reads no directory, asserts nothing about any tree and never exits, so the COVERAGE LOST it feeds belongs to its importers, each of which already carries its own — assert-apps-gov-in-apk.mjs over apksigner and aapt2, assert-artifact-signed-apple.mjs over codesign and assert-xcode-floor.mjs over xcodebuild. `toolOutputLines` moved here from assert-apps-gov-in-apk.mjs UNCHANGED on 2026-09-24 (row O-APPLE-GUARDS-PARSE-HAND-WRITTEN-OUTPUT), when the two Apple guards needed the block apps-gov-in already printed; that guard re-exports it, and its test asserts the re-export IS the moved function. Its own failing cases are in test/apps-gov-in-apk.test.mjs (25 lines in, 20 shown, the remainder marked), and each Apple guard\'s suite reds when its COVERAGE LOST stops printing the block. It sits flat in tooling/ci because the stray-.mjs check above (correctly) treats a subdirectory as a guard escaping the scan.',
  ],
  [
    'licence-cross-assert.mjs',
    'is not a guard: it is the ONE seam check between the two licence registers (tooling/legal/content-licence-register.json and tooling/legal/asset-register.json), imported by the two guards that own them — assert-licence-register.mjs and assert-content-licences.mjs — so a disagreement turns BOTH red. It returns FINDINGS in `problems` and its could-not-look stops (a register missing or unparseable, either side empty, a compared field renamed away) in `lost`, as bare reasons; it never exits and never reads the process arguments. Each caller prefixes every `lost` entry with CROSS-ASSERT COVERAGE LOST and exits 2 through its OWN coverageLost when those stops are all the run has, 1 when a finding sits beside them — the chassis-delegation `{ lost }` channel, for the same reason: the exit belongs to the duty that could not be done. Until 2026-09-18 it pushed the marker into `problems` itself, so both callers exited 1 on a seam they never read (O-EXIT2-CONVENTION-GAP). Its failing cases are in test/licence-register.test.mjs — every stop asserted exit 2 on a tree with nothing else wrong, a real finding kept at 1, and the same disagreement asserted red from both sides. It sits flat in tooling/ci because the stray-.mjs check above (correctly) treats a subdirectory as a guard escaping the scan.',
  ],
  [
  ...
  [
    'flutter-release-build.mjs',
    'is not a guard: it is the ONE composition of a release `flutter build` — the channel stamp, the store SDK key of the channel\'s rail (`purchaseRails.storeKeyDefine`), the symbols directory, the build number, the `--lane pr` blanking and `apiBaseUrl`, the API base rule — which twenty-one workflow lines each typed by hand until 2026-09-25 (O-FLUTTER-BUILD-TYPED-PER-LINE, part 1 of 3). It performs an action (`flutter build`, or `--print`) rather than scanning anything, in the same class as record-deployment.mjs, so there is no tree it could silently cover less of. What it CAN lose is a decision, and those are pinned by exact argv in test/flutter-release-build.test.mjs, beside a case that reds on any `${{` in a composed argv (secrets travel by env NAME) and one that proves run mode refuses an unset name before flutter starts. Two readers import it: assert-stamp-text-fidelity.mjs grades a stamped app\'s API base with `apiBaseUrl`, and workflow-scan.mjs\'s census composes each workflow call to it, so every census reader grades the build the call makes — each of those carries its own COVERAGE LOST over what it read. It sits flat in tooling/ci because the stray-.mjs check above (correctly) treats a subdirectory as a guard escaping the scan.',
  ],
]);
...
const NO_NEGATIVE_TEST_NEEDED = new Map([
  ...
  [
    ...
    'is the e2e harness, not a guard: its whole purpose is the live assertion, which e2e.yml performs nightly against real data. The fixture-testable half — whether the Workers trust the run\'s issuer (keyed on the auth target until 2026-09-25) and the verdict for each — was split out on 2026-09-22 into tooling/e2e/auth_target_expectation.mjs, which tooling/ci/test/e2e-verify-target.test.mjs drives through every branch; the same test reads this file (never runs it) to hold its wiring.',
  ],
  [
