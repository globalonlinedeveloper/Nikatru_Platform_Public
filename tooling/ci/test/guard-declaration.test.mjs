...
test('(1) public side: the COMMITTED blob is read, a corrupted working tree is not, and only hook:["public"] entries are selected', () => {
  ...
  try {
    const P = corpusRepo(join(base, 'Fixture_Private'), { guards: validEntries() });
...
test('(2) private side: the STAGED blob is read, not HEAD and not the working tree, and hook:["private"] entries are selected', () => {
  ...
  try {
    ...
    const P = corpusRepo(join(base, 'Fixture_Private'), committed);
    ...
    write(declPath(P), `${JSON.stringify(staged, null, 2)}\n`);
    ...
    assert.equal(atHead.some((e) => e.id === 'fixture-staged-only'), false, 'the staged entry is already in HEAD, so this case cannot tell the two blobs apart');
...
test('(3) a missing blob while the corpus is present is a refusal naming the blob and the root, on both sides', () => {
  ...
  try {
    ...
    write(declPath(P), `${JSON.stringify(validEntries())}\n`);
    ...
    write(declPath(bare), `${JSON.stringify(validEntries())}\n`);
...
test('(4) an unparseable blob, or one with no entry list, is a parse refusal', () => {
  const base = mkdtempSync(join(tmpdir(), 'guard-decl-parse-'));
  try {
    const notJson = corpusRepo(join(base, 'A_Private'), '{ "guards": [ \n');
    ...
    assert.match(e2.message, /holds no entry list/);

    const twoLists = corpusRepo(join(base, 'C_Private'), { guards: validEntries(), entries: validEntries() });
    const e3 = refusalOf(() => loadGuardDeclaration(twoLists, 'public'));
    assert.equal(e3.limb, 'parse');
    assert.match(e3.message, /ambiguous/);

    // CONTROL — the same entries as a bare array, and under `entries`, load.
    assert.equal(loadGuardDeclaration(corpusRepo(join(base, 'D_Private'), validEntries()), 'public').entries.length, validEntries().length);
    assert.equal(loadGuardDeclaration(corpusRepo(join(base, 'E_Private'), { entries: validEntries() }), 'public').entries.length, validEntries().length);
  } finally {
    rmSync(base, { recursive: true, force: true });
  }
});
...
test('(5) an entry without id, file or hook is refused, and every malformed entry is named', () => {
  ...
  try {
    ...
    shapes[17].args = '--held';
    const Q = corpusRepo(join(base, 'Shapes_Private'), shapes);
...
test('(6) a pinned guard absent, or declared for one side only, is refused — assert-platform-state removed is COVERAGE LOST', () => {
  ...
  try {
    ...
    const P = corpusRepo(join(base, 'Fixture_Private'), without);
    ...
    const Q = corpusRepo(join(base, 'OneSided_Private'), oneSided);
...
test('(7) an exported GIT_DIR and GIT_INDEX_FILE naming another repository do not redirect the read', () => {
  ...
  try {
    const P = corpusRepo(join(base, 'Fixture_Private'), validEntries());
    ...
    const D = corpusRepo(join(base, 'Decoy_Public'), validEntries().filter((e) => e.id !== 'check-dod-sync'));
    ...
    withEnv(poison, () => {
      ...
      assert.equal(JSON.parse(raw.stdout).some((e) => e.id === 'check-dod-sync'), false, 'the unguarded `git -C` did NOT read the decoy, so this environment does not reproduce the hook and the assertions above prove nothing');
    });
  } finally {
    rmSync(base, { recursive: true, force: true });
  }
