...
describe('the guard says YES on the tree as it is', () => {
  test('the REAL repository — 19 reachable, 19 swept, 0 printed, exit 0', () => {
    ...
    assert.match(out, /19 swept by 1 a11y test file\(s\) across 81 case\(s\)/);
    ...
    // started is worth holding, and so is `contrast ×0` still being the one
    // that has not.
    assert.match(out, /sweep families used: naked-controls ×24, tap-target ×19, contrast ×0/);
  });
  ...
  test('M10 · a NEW sweep is counted — without this, every failure below is vacuous', () => {
    ...
    assert.equal(printedUnswept(out).length, 0);
    assert.match(out, /20 swept by 1 a11y test file\(s\) across 82 case\(s\)/);
    assert.match(out, /0 unswept and PRINTED/);
  });
});
...
describe('an empty scan is COVERAGE LOST, never a pass', () => {
  ...
  test('M5 · every sweep helper is renamed — 81 cases, not one sweep', () => {
    const root = tree();
    ...
    const { code, out } = run(root);
    assert.equal(code, 1, out);
    assert.match(out, /COVERAGE LOST — 81 a11y case\(s\) were parsed .* and NOT ONE of them calls a sweep/s);
  ...
  test('M8 · four cases are deleted while every sweep survives', () => {
    ...
    const { code, out } = run(root);
    assert.equal(code, 1, out);
    assert.match(out, /COVERAGE LOST — only 77 a11y case\(s\) .* the checked-in floor is 81/s);
