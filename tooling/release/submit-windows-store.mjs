...
// company/runbooks/store-submission-windows.md. Limb (iii) — a submission
...
if (SUBMIT) {
  ...
  console.error('       company/runbooks/store-submission-windows.md');
  console.error('');
...
if (DRY_RUN) {
  ...
  console.log(`   Console-only steps that must happen first: ${channel.submission?.runbook ?? 'company/runbooks/store-submission-windows.md'}`);
  process.exit(0);
}
