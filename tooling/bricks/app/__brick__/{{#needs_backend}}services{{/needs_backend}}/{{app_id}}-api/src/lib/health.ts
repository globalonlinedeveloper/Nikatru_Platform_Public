...
//
// 🔴 THE STATE THIS REPLACES, AND THIS TEMPLATE STAMPED IT. A stamped Worker
// answered `/v1/health` with `ok: true` as a LITERAL, and two live probes rest
// on that field:
//   · `tooling/ops/post-deploy-smoke.mjs --require-ok` reads `ok` and asserted a
//     constant — `judgeOk` parsed a body whose answer was decided at compile time;
//   · a GlitchTip body assertion on `"ok":true` — the same constant.
// So a fresh app could have APP_DB unreachable, PLATFORM_DB refusing reads, or
// the Supabase JWKS fetch failing (every authenticated route 401s) and stay
// green on day one. That is the `Ping` monitor shape at the deepest point in the
// stack: it proves a socket opened and nothing else.
//
// ── ⚠️ "SHARED" MEANS DUPLICATED, NOT IMPORTED ───────────────────────────────
// Every Worker under `services/` carries a BYTE-IDENTICAL copy of this file, for
// the same reason `lib/d1.ts` is duplicated: the Workers are separate npm
// packages with separate deploys and no module boundary between them. That
// duplication is not on trust — `services/platform/test/twinned-worker-modules
// .test.ts` derives its subject set from `services/*/src/lib/*.ts` and holds
// every declaration below identical across every carrier, so THE STAMPED COPY
// JOINS THAT SET THE DAY THE APP IS STAMPED: a declaration edited here and not
// in the other carriers is a RED BUILD, not a discovery. Nothing Worker-specific
// may live here — the dependency LIST is assembled in `src/index.ts`, which is
// where carriers legitimately differ.
//
