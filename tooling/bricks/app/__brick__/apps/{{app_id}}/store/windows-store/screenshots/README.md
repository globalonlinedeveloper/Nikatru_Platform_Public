# Screenshots — {{{short_name}}} · `windows-store`
...
The Microsoft Store's rules for this gallery ARE declared, in
`tooling/channel-register.json` under
`storeMetadataContract.perChannel["windows-store"].graphicAssets.screenshots`,
and they are enforced against the pixels by
`tooling/ci/assert-listing-assets.mjs`. Read them there rather than here: each
number carries a dated `source` saying where it came from, and which of them are
this repository's own HOUSE RULES rather than the store's — a distinction a copy
in prose loses. Note what the register does NOT declare: there is no maximum
dimension, because none was read from a Microsoft page and an invented ceiling
...
2. Capture the frames. `tooling/store/capture-play-screenshots.mjs --channel
   windows-store` does it against a live build of the REAL Windows binary and
   ...
   `.github/workflows/store-screenshots.yml` with `channel: windows-store` to
