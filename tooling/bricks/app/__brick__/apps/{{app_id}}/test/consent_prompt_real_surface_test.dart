...
// REFUSAL half, which no inherited assertion covered.
//
// 🔴 WHAT `chassis_properties_test.dart` ALREADY COVERS AND WHY IT IS NOT
// ENOUGH. Its "the app root asks for consent, and Allow opens the rail" property
// taps ALLOW ONLY. That direction is the one [pipeline C-6] was about, so it got
// the attention — but it means a stamped app inherited ZERO evidence that a
// refusal dismisses the prompt, is recorded, survives a relaunch, or actually
// stops collection. Every one of those is a DPDP obligation, and every one of
// them fails SILENTLY: refusing is what a broken rail already does, so nothing
// goes red when the refusal path is the thing that is broken.
//
// The claims this file makes assertable for every stamp:
...
//     silence. The append-only record needs the "no" as much as the "yes";
...
//     TAP — those fields are otherwise only ever proven by calling
//     `applyConsentDecision` directly, which is the seam, not the surface.
//
// 🔴 FINDERS ARE THE CONTROLS, NOT THE BODY COPY. The prompt's title and privacy
// sentence are app-owned strings a stamp is expected to restyle and re-word; the
// two BUTTONS are the chassis contract, and their widget types carry the
// equal-prominence rule the prompt exists to honour. A test pinned to the prose
// would break on a copy edit and prove nothing about behaviour.
// ─────────────────────────────────────────────────────────────────────────────
...
///
/// `AppConfig.isBackendLive` is compile-time and a fresh stamp is born with it
/// off, so without this override the prompt never mounts and none of the paths
/// below exist to be driven.
...
/// the app root twice reuses the existing element, so the analytics gate's state
...
void main() {
  group('the real consent prompt — the DECLINE path', () {
    ...
    testWidgets('a refusal raises nothing — the unmount races the decision', (
      ...
      expect(
        ...
        isNull,
        reason:
            'the prompt\'s answer handler is deliberately un-awaited, so the '
            'decision path keeps running AFTER the prompt it was tapped on is '
            'unmounted. A `ref` touched past that point throws in debug and '
            'takes the rest of the decision — the upload and the invalidate — '
            'down with it',
      );
      expect(
  ...
  group('the real consent prompt — what an ALLOW actually records', () {
    testWidgets('a real tap stamps the policy version, platform and anon id', (
      ...
      // The set is `analyticsPlatformName`'s whole range. Asserting membership
      // rather than one value keeps this true on all six lanes; asserting the
      // range at all is what would catch the field going empty or unset.
      expect(
        artifact.platform,
        isIn(const <String>[
          'web',
        ...
        reason:
            'an artifact that cannot say where the consent was given is a '
