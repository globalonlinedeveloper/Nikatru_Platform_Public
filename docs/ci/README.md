...
`ci.yml` runs **20** jobs, and **17** of them are in `ci-gate`'s `needs`.
...
| `ci-gate` | ci-gate | `lane-workers`, `guard-meta`, `guards-platform`, `guards-legal`, `guards-store`, `guards-chassis`, `security-scan`, `site-tokens`, `site-shared`, `content-gate`, `app-brick`, `sites`, `workspace-gate`, `prepare`, `app-dryrun`, `android-artifacts`, `extensions` | yes | — (the aggregate: the single required status check on `main`) |
...
<!-- why: GENERATED by node tooling/ci/gen-ci-map.mjs --write. Never hand-edit. -->
The workflows read **51** secret names (`GITHUB_TOKEN` is not counted: GitHub issues it to every run).
...
| `API_BASE_URL` | `build-platforms.yml`, `ci.yml`, `deploy-web.yml`, `e2e.yml`, `submit-appstore.yml`, `submit-play.yml`, `submit-snap.yml`, `submit-windows-store.yml` |
...
| `APP_STORE_CONNECT_IOS_APP_ID` | `submit-appstore.yml` |
...
| `APP_STORE_CONNECT_MACOS_APP_ID` | `submit-appstore.yml` |
| `APP_STORE_CONNECT_PRIVATE_KEY` | `apple-expiry-write.yml`, `ops-watch.yml`, `submit-appstore.yml` |
| `CLOUDFLARE_ACCOUNT_ID` | `ci.yml`, `deploy-sandbox.yml`, `deploy-web.yml`, `deploy-workers.yml`, `e2e.yml`, `ops-watch.yml`, `rollback.yml`, `store-screenshots.yml` |
| `CLOUDFLARE_API_TOKEN` | `ci.yml`, `deploy-sandbox.yml`, `deploy-web.yml`, `deploy-workers.yml`, `e2e.yml`, `ops-watch.yml`, `rollback.yml`, `store-screenshots.yml` |
...
| `MS_STORE_PRODUCT_ID` | `submit-windows-store.yml` |
...
| `RENOVATE_TOKEN` | `name-clearance.yml`, `renovate.yml`, `site-drift-repair.yml` |
...
| `SUPABASE_ANON_KEY` | `build-platforms.yml`, `ci.yml`, `deploy-web.yml`, `deploy-workers.yml`, `e2e.yml`, `ops-watch.yml`, `store-screenshots.yml`, `submit-appstore.yml`, `submit-play.yml`, `submit-snap.yml`, `submit-windows-store.yml` |
| `SUPABASE_PAT` | `ops-watch.yml` |
| `SUPABASE_PROJECT_REF` | `ops-watch.yml` |
| `SUPABASE_SERVICE_ROLE_KEY` | `e2e.yml`, `store-screenshots.yml` |
| `SUPABASE_URL` | `build-platforms.yml`, `ci.yml`, `deploy-web.yml`, `e2e.yml`, `ops-watch.yml`, `store-screenshots.yml`, `submit-appstore.yml`, `submit-play.yml`, `submit-snap.yml`, `submit-windows-store.yml` |
...
<!-- why: GENERATED by node tooling/ci/gen-ci-map.mjs --write. Never hand-edit. -->
The tree uses **5** third-party actions and **8** GitHub-owned ones, read from every `uses:` in `.github/workflows` and in the composite actions under `.github/actions`. Each third-party one runs only while the `allowed_actions` setting allows it: named on its allowlist, or published by a verified creator.

| action | owner | used by |
|---|---|---|
| `cloudflare/wrangler-action` | third-party | `deploy-sandbox.yml`, `deploy-web.yml`, `deploy-workers.yml` |
...
| `trufflesecurity/trufflehog` | third-party | `trufflehog.yml` |
| `actions/cache` | GitHub | `ci.yml`, `ops-watch.yml` |
| `actions/checkout` | GitHub | `apple-expiry-write.yml`, `build-platforms.yml`, `ci.yml`, `codeql.yml`, `deploy-sandbox.yml`, `deploy-web.yml`, `deploy-workers.yml`, `e2e.yml`, `extensions-ci.yml`, `extensions.yml`, `lane-workers.yml`, `name-clearance.yml`, `ops-watch.yml`, `redeploy-stranded.yml`, `rollback.yml`, `site-drift-repair.yml`, `store-screenshots.yml`, `submit-appstore.yml`, `submit-play.yml`, `submit-snap.yml`, `submit-windows-store.yml`, `symbolication-proof.yml`, `trufflehog.yml` |
...
| `actions/upload-artifact` | GitHub | `build-platforms.yml`, `ci.yml`, `e2e.yml`, `extensions-ci.yml`, `extensions.yml`, `name-clearance.yml`, `site-drift-repair.yml`, `store-screenshots.yml`, `submit-appstore.yml`, `submit-play.yml`, `submit-snap.yml`, `submit-windows-store.yml`, `symbolication-proof.yml` |
...
<!-- why: GENERATED by node tooling/ci/gen-ci-map.mjs --write. Never hand-edit. -->
**24** workflows. A workflow's page is `docs/ci/<workflow>.md`; `ci.yml`'s is this one.
...
| this page | `.github/workflows/ci.yml` | CI | `push`, `pull_request` | 20 |
...
| [`site-drift-repair.md`](site-drift-repair.md) | `.github/workflows/site-drift-repair.yml` | Site drift repair | `push` | 1 |
