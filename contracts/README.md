...
| `app-id/` | the APP ID rule — the pattern, the length bounds and the reserved words an `app_id` must meet before it becomes a Worker, a database, a DNS label, a Dart package and a store identity. See `app-id/README.md` | `tooling/scripts/provision-backend.mjs`, `tooling/ci/assert-store-identity.mjs` and `tooling/ci/assert-catalog-contract.mjs` import `app-id.js`; the brick's `pre_gen.dart` reads the generated `app-id.json` |
...
| `app-id/app-id.js` ↔ `app-id/app-id.json` ↔ its four callers ↔ `services/platform/src/config.ts` `APP_ID_PATTERN` ↔ the two app-yaml schemas and `release.schema.json` `unit` | `tooling/ci/assert-app-id-contract.mjs` — the JSON is what `app-id/generate.mjs` writes, every id the contract accepts passes the Worker's filter, each caller uses the contract and carries no app-id regex of its own, and each schema accepts no id the contract refuses | `ci.yml` · guards-store |
