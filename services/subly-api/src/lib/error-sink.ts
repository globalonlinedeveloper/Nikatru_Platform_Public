...
//
// ⚠️ A DELIBERATE DUPLICATE of services/platform/src/lib/error-sink.ts, for the
// same reason `lib/d1.ts` is duplicated: the two Workers are separate npm
...
// keeps no searchable log history, so an unhandled error in this Worker — the
// one holding the flagship app's REAL USER ROWS — produced exactly one
// artefact: a 500 the client saw. `grep -rn "sentry\|glitchtip" services/`
// returned zero hits while the Flutter app's crashes had been reaching
// GlitchTip since July.
//
