...
describe('🔴 THE FAIL-CLOSED INTERLOCK — the sweep cannot outrun the rollup', () => {
  it('THE MUTATION: with NO rollup run, the sweep deletes NOTHING even when every row is "old"', async () => {
    ...
    await retentionSweep(envOf(db), { events: 1, events_daily: null, provider_notifications: null, signups: null, content_reports: null, ext_codes: null, ext_devices: null }, NOW);
...
describe('events_daily is itself swept, on age alone', () => {
  it('deletes rollup rows past their period and keeps newer ones', async () => {
    ...
    const periods: RetentionPeriods = { events: null, events_daily: 30, provider_notifications: null, signups: null, content_reports: null, ext_codes: null, ext_devices: null };
