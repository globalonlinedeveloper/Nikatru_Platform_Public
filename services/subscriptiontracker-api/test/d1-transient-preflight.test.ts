 ...
 * INTO `batch()` — the same change as services/platform's copy, for the same
 * reason. The pragma reads now travel as ONE batch; a batch carrying any matching
 * statement is ONE execution and fails as one, which is the unit D1 resets and
 * `withD1Retry` re-sends. The cases themselves are unchanged.
 */
const INNER = Symbol('flaky-inner');
type Wrapped = { [INNER]: ReturnType<SqliteD1['prepare']> };
...
class FlakyD1 {
  ...
  private wrap(stmt: ReturnType<SqliteD1['prepare']>) {
    // eslint-disable-next-line @typescript-eslint/no-this-alias
    const self = this;
    return {
      [INNER]: stmt,
      bind: (...args: unknown[]) => self.wrap(stmt.bind(...args) as ReturnType<SqliteD1['prepare']>),
