...
void main() {
  ...
  void answer(Object? Function(MethodCall call) handler) {
    messenger.setMockMethodCallHandler(ageSignalsChannel, (MethodCall call) async {
      calls.add(call);
      return handler(call);
    });
  }
  ...
  group('Play Age Signals adapter', () {
    test('SHARED 18+ is adult', () async {
      answer((_) => <String, Object?>{'accessStatus': 'SHARED', 'ageLower': 18, 'ageUpper': null});
      expect(await play(), const core.AdultAgeSignal());
    });
    test('SHARED 13–15 is below adult', () async {
      answer((_) => <String, Object?>{'accessStatus': 'SHARED', 'ageLower': 13, 'ageUpper': 15});
    ...
    test('NOT_SHARED is declined', () async {
      ...
      expect(await play(), const core.NoAgeSignal(core.NoAgeSignalReason.declined));
    });
    test('VERIFICATION_REQUIRED is unavailable (proceeds on the declaration)', () async {
      answer((_) => <String, Object?>{'accessStatus': 'VERIFICATION_REQUIRED'});
      expect(await play(), const core.NoAgeSignal(core.NoAgeSignalReason.unavailable));
    });
    test('the native error flag is error', () async {
      answer((_) => <String, Object?>{'error': true});
      expect(await play(), const core.NoAgeSignal(core.NoAgeSignalReason.error));
    });
  ...
  group('Apple Declared Age Range adapter', () {
    test('no API on this OS (eligible null) is unavailable', () async {
      ...
      expect(await apple(), const core.NoAgeSignal(core.NoAgeSignalReason.unavailable));
    });
    test('not eligible is notEligible', () async {
      answer((_) => <String, Object?>{'eligible': false});
      expect(await apple(), const core.NoAgeSignal(core.NoAgeSignalReason.notEligible));
    });
    test('sharing lowerBound 18 is adult', () async {
      answer((_) => <String, Object?>{'eligible': true, 'response': 'sharing', 'lowerBound': 18, 'upperBound': null});
      expect(await apple(), const core.AdultAgeSignal());
    });
    test('sharing below the gate is below adult', () async {
      answer((_) => <String, Object?>{'eligible': true, 'response': 'sharing', 'lowerBound': null, 'upperBound': 17});
      expect(await apple(), const core.BelowAdultAgeSignal());
    });
    test('declinedSharing is declined', () async {
      answer((_) => <String, Object?>{'eligible': true, 'response': 'declinedSharing'});
      expect(await apple(), const core.NoAgeSignal(core.NoAgeSignalReason.declined));
    });
    test('the native error flag and a PlatformException are error', () async {
      answer((_) => <String, Object?>{'error': true});
      expect(await apple(), const core.NoAgeSignal(core.NoAgeSignalReason.error));
      answer((_) => throw PlatformException(code: 'boom'));
      expect(await apple(), const core.NoAgeSignal(core.NoAgeSignalReason.error));
    });
  });
  ...
  group('storeAgeSignalSourceFor — every host', () {
    test('android reads Play', () {
      expect(storeAgeSignalSourceFor(core.AgeSignalHost.android), isA<PlayAgeSignalSource>());
    });
    test('ios reads Apple', () {
      expect(storeAgeSignalSourceFor(core.AgeSignalHost.ios), isA<AppleAgeSignalSource>());
    });
    test('macos, windows, linux, web and other read no signal (noApiOnTarget)', () async {
      expect(await storeAgeSignalSourceFor(core.AgeSignalHost.macos).read(), const core.NoAgeSignal(core.NoAgeSignalReason.noApiOnTarget));
      expect(await storeAgeSignalSourceFor(core.AgeSignalHost.windows).read(), const core.NoAgeSignal(core.NoAgeSignalReason.noApiOnTarget));
      expect(await storeAgeSignalSourceFor(core.AgeSignalHost.linux).read(), const core.NoAgeSignal(core.NoAgeSignalReason.noApiOnTarget));
      expect(await storeAgeSignalSourceFor(core.AgeSignalHost.web).read(), const core.NoAgeSignal(core.NoAgeSignalReason.noApiOnTarget));
      expect(await storeAgeSignalSourceFor(core.AgeSignalHost.other).read(), const core.NoAgeSignal(core.NoAgeSignalReason.noApiOnTarget));
    });
  });

  group('🔴 read, decided on, discarded — nothing logged, stored or sent', () {
    test('a read prints and debugPrints nothing, on either adapter, on any answer', () async {
      ...
      await runZoned(
        () async {
          answer((_) => <String, Object?>{'accessStatus': 'SHARED', 'ageLower': 13, 'ageUpper': 15});
          await play();
          answer((_) => <String, Object?>{'eligible': true, 'response': 'sharing', 'lowerBound': 18});
          await apple();
          answer((_) => throw PlatformException(code: 'boom', message: 'age 13-15'));
          await play();
        },
        zoneSpecification: ZoneSpecification(print: (_, __, ___, String line) => printed.add(line)),
      );
      expect(printed, isEmpty);
    });

    test('the only message sent to the native side is one argument-less `read`', () async {
      answer((_) => <String, Object?>{'accessStatus': 'SHARED', 'ageLower': 18});
      await play();
    ...
    test('the native sources contain no logging, storage or network call', () {
      final Map<String, String> sources = <String, String>{
        'android': File('android/src/main/kotlin/com/nikatru/age_signals/AgeSignalsPlugin.kt').readAsStringSync(),
        'ios': File('ios/nikatru_age_signals/Sources/nikatru_age_signals/AgeSignalsPlugin.swift').readAsStringSync(),
      };
      ...
      for (final MapEntry<String, String> e in sources.entries) {
        ...
        final String code = e.value
            ...
            .join('\n');
        expect(forbidden.hasMatch(code), isFalse, reason: '${e.key}: ${forbidden.firstMatch(code)?.group(0)}');
      }
    });
  });
}
